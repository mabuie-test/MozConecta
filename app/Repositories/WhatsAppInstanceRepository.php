...
final class WhatsAppInstanceRepository extends BaseRepository
{
    ...
    public function update(int $tenantId, int $id, array $payload): void
    {
        ...
        $metadata = [];
        if ($instance && !empty($instance['metadata_json'])) {
            $decoded = json_decode((string) $instance['metadata_json'], true);
            $metadata = is_array($decoded) ? $decoded : [];
        }

        $metadata['connection_method'] = 'linked_devices_unofficial';
        $metadata['official_channel'] = false;
        $metadata['risk_notice'] = 'Linked Devices/WhatsApp Web via QR/pairing é não oficial e pode causar banimento/instabilidade em SaaS comercial.';

        $this->execute('UPDATE whatsapp_instances SET name=:name, phone_number=:phone_number, pairing_mode=:pairing_mode, metadata_json=:metadata_json, updated_at=NOW() WHERE id=:id AND tenant_id=:tenant_id', [
            'name' => $payload['name'],
