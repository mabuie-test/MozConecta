...
final class DebitoEmolaProvider implements PaymentProviderInterface
{
    ...
    {
        $walletId = trim((string) env('DEBITO_WALLET_ID_EMOLA', ''));
        if ($walletId === '') {
            throw new \RuntimeException('DEBITO_WALLET_ID_EMOLA não configurado.');
        }

        $body = $this->buildPayload($payload);
        $endpoint = '/api/v1/wallets/' . rawurlencode($walletId) . '/c2b/emola';

        $client = new DebitoClient($this->logger, $this->auth);
        $response = $client->request('POST', $endpoint, $body, true);

        return $this->normalizeResponse($response, 'emola', $walletId);
    }
    ...
    public function checkStatus(string $debitoReference): array
    {
        ...
        return $this->normalizeResponse($response, 'emola', trim((string) env('DEBITO_WALLET_ID_EMOLA', '')));
    }
    ...
    private function buildPayload(array $payload): array
    {
        ...
        if (!preg_match('/^(258)?8[4-7][0-9]{7}$/', $msisdn)) {
            throw new \InvalidArgumentException('MSISDN inválido para eMola. Use formato nacional válido.');
        }
        if ($amount < 1) {
            throw new \InvalidArgumentException('Valor inválido para eMola (mínimo 1).');
        }
        if ($referenceDescription === '') {
            throw new \InvalidArgumentException('reference_description é obrigatório para eMola.');
        }
